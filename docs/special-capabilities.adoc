...
By default saved log files are named `<session-id>.txt` where `<session-id>` is a unique identifier of Selenium session.
To provide custom log file name specify:

.Type: string
----
logName: "my-cool-log.txt"
----

WARNING: It is important to add `txt` file extension.
